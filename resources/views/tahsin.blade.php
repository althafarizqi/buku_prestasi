...
<div class="card card-primary">
    <div class="card-header">
        <div class="user-block">
            <h3 class="card-title">Input Nilai Tahsin</h3>
        </div>
    ...
    <div class="card-body">
        <form action="{{ route('tahsin.index') }}" method="get">
...
<div class="card card-success">
    <div class="card-header">
        <div class="user-block">
            <h3 class="card-title">Daftar Nilai Tahsin</h3>
        </div>
        <div class="card-tools">
            <button type="button" class="btn btn-tool" data-card-widget="collapse">
                <i class="fas fa-minus"></i>
        ...
        <div class="table-responsive">
            <table class="table">
                ...
                <tbody class="text-center">
                    @php
                    $groupedTahsinDetail = $tahsindetail->groupBy('nama');
                    @endphp

                    @foreach ($groupedTahsinDetail as $nama => $group)
                    @foreach ($group as $key => $tahsin)
                    <tr>
                        <td>
                            @if ($key === 0)
                        ...
                        <td>{{$tahsin->tingkat}}</td>
                        <td>{{$tahsin->fashohah}}</td>
                        <td>{{$tahsin->tajwid}}</td>
                        <td>{{$tahsin->kelancaran}}</td>
                        <td>{{$tahsin->rata_rata}}</td>
                        <td>
                            <a href="tahsindetail/{{$tahsin->id}}/edit"
                                class="d-inline badge badge-pill badge-warning">Edit</a>
                                <form class="d-inline" action="tahsindetail/{{$tahsin->id}}" method="post">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
...
<script>
    ...
    let tr = `<tr id="input-tr-${i}">
        <td class="pb-0">
            <div class="form-row">
                <div class="col">
                    <select name="tingkat[]" class="form-control" style="width: 100%;" required oninvalid="this.setCustomValidity('Data tidak boleh kosong')" oninput="setCustomValidity('')">
