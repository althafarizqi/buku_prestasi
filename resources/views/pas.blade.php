...
<div class="card card-primary">
    <div class="card-header">
        <div class="user-block">
            <h3 class="card-title">Input Nilai Ujian Akhir Semester</h3>
        </div>
    ...
    <div class="card-body">
        <form action="{{ route('pas.index') }}" method="get">
...
<div class="card card-success">
    <div class="card-header">
        <div class="user-block">
            <h3 class="card-title">Daftar Nilai Ujian Akhir Semester</h3>
        </div>
        <div class="card-tools">
            <button type="button" class="btn btn-tool" data-card-widget="collapse">
                <i class="fas fa-minus"></i>
        ...
        <div class="table-responsive">
            <table class="table">
                ...
                <tbody class="text-center">
                    @php
                    $groupedPasDetail = $pasdetail->groupBy('nama');
                    @endphp

                    @foreach ($groupedPasDetail as $nama => $group)
                    @foreach ($group as $key => $pas)
                    <tr>
                        <td>
                            @if ($key === 0)
                        ...
                        <td>{{$pas->fashohah}}</td>
                        <td>{{$pas->tajwid}}</td>
                        <td>{{$pas->kelancaran}}</td>
                        <td>{{$pas->rata_rata}}</td>
                        <td>
                            <a href="pasdetail/{{$pas->id}}/edit" class="badge badge-pill badge-warning">Edit</a>
                            <form class="d-inline" action="pasdetail/{{$pas->id}}" method="post">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
