...
@section('content')
<form action="{{ route ('tahsindetail.update', ['id' => $detail->id]) }}" method="post">
    @method('PUT')
    @csrf
    <table class="table">
        ...
        <tbody align="center">
            <tr>
                <td><span class="text-left text-bold">{{$tahsindetail->first()->nama}}</span></td>
                <td>
                    <select name="tingkat" class="form-control" style="width: 100%;">
                        <option value="{{$detail->tingkat}}" selected>{{$detail->tingkat}}</option>
                ...
                <td class="row">
                    ...
                    <div><a class="ml-1 btn btn-warning" href="javascript:history.back()">Batal</a></div>
                </td>
            </tr>
        </tbody>
